...
Describe "Save-XLFile/Sheet" {
    Context "File already exists" {
        ...
        It "Should be updated" {
            $path | Should -Exist
            Start-Sleep -Milliseconds 500
            Save-XLFile -Sheet (Get-XLSheet -File $xl -Index 0)
            $fileAfter = Get-Item -Path $path
            $path | Should -Exist
            $lastWritTimeAfter = (Get-Item -Path $path).LastWriteTime
    ...
    Context "File doesn't exist" {
        ...
        It "Should return be created" {
            ...
            $null = Add-XLSheet -File $file -Name "X"
            $path | Should -Not -Exist
            Save-XLFile -Sheet (Get-XLSheet -File $file -Index 0)
            $path | Should -Exist
        }
    }
    Context "Sheet has no owner" {
        ...
        It "Should throw" {
            [XLSheet]$sheet = (Get-XLSheet -File $file -Index 0).Worksheet
    ...
    Context "With -PassThru" {
        ...
        It "Should return [XLSheet]" {
            Save-XLFile -Sheet (Get-XLSheet -File $file -Index 0) -PassThru | %{$_ -is [XLSheet]} | Should -Be $true
        }
    }
}
